...
Our data is made up of 745,612 different transactions in the AAVE v2 lending protocol. These transactions are a total of 7 types (deposit, redeem, borrow, repay, swap, liquidation, collateral). There are a total of 53 different coins in the data. These are divided into stablecoins, non-stable coins, and Amm coins. For each transaction, we have a total of 33 columns which record everything about the transaction from the date to the user and the amount of money involved.

# Results

Problem 1- We are able to cluster users into groups that allow us the see the behavior of users who are often liquidated. Most users are not overly active and prefer to let their coins sit and make whatever interest they can


Problem 2-Although the number of active users in AAVE has not increased, people are using larger amounts of money in their transactions. AAVE;s market is heavily subject to shock events that will briefly disrupt transaction patterns.

Problem 3- Stable coins are borrowed at much higher rates than other coins, and their is a wide variety of uses for these other non-stable coins. Stable coins all seem to high fairly similar patterns in their utilizations rates.
...
I created a new dataframe where each row represented an individual user. Each user has certain statistics for each type of transaction, as well as the amount of time they have been active. The first few rows are shown below.
...
df.users<- df%>%dplyr::group_by(user, user_alias)%>%
...
From here, I used pca and kmeans clustering to build the actual clusters.

### Results

With a bit of work we can select the columns we would like to cluster on, as well as manually scaling the database. To preserve the original users dataframe, we save the scaled data separately. Also, I also removed outlier users who we either know or suspect of being SMART Contracts. They throw off clustering in a way we do not want. From their, we can run PCA.
...
Plotting the PCA allows us to chose the number of cluster we wish to have. In this case, three appears optimal, however I chose to work with 5. If I chose 3, we lose some of the distinctions between clusters and they appear to mean less. Thus let's look at what the clusters are and how users are distributed.
...
First, we can see that it appears most users get put into cluster 3. This should represent our more average user, as opposed to cluster 2. Until we see the cluster centers we cannot draw any real conclusions.
...
ggplot(unique_users) + geom_line(aes(x = date, y = num_users))+
  ...
  geom_ma(n = 30, aes(x=date, y = num_users, color = "7 Day Moving Average"))
```
...
This confirms what I found earlier about many users not being very active. The majority of users have less than 10 transactions per active week. However, there are many outliers that are very active and fall into my first cluster. However, this does do a good job of showing why cluster 3 is the largest. 
