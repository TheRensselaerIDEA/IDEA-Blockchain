...
All the code for the graphs can be found in vakhrr_final.Rmd.
...
The work was done based on the data collected from AAVE protocol. AAVE is a cryptocurrency protocol, which was designed for users to take loans in various cryptocurrencies. AAVE allows users to make 7 types of transactions: deposit,swap,repay,borrow,redeem,liquidation, and, collateral. This paper focuses mostly on liquidation transaction.

In order, for liquidation transaction to happen, some user has to first make a borrow call. During borrow, a user can get any type of cryptocurrency from the protocol, but they have to put some collateral (in any other cryptocurrency) to support the loan. The borrowed amount (principal) has to be less in value than the collateral for that loan. The important idea is that collateral and principal can be in different cryptocurrencies, yet when comparing them the protocol uses their value in WETH (Wrapped Ether).

In the event, when the value of the collateral is significantly smaller than that of the principal (it may change due to changes in prices of either cryptocurrency), the loan can be liquidated. Liquidation call can be initiated by any AAVE user. During liquidation, the user pays back part of the loan in the original principal currency and receives part of the collateral at discounted price. Liquidation transaction was designed in such a way to incentivize users to make liquidation calls (to make some profit) and to discourage users from making loans with insecure collateral. 

Therefore, it is possible to study AAVE protocol not only from the perspective of loans and risk, but also from the perspective of liquidations and different approaches to make profit out of them.

# Original Data

The data was prepared based on the original transactions data. The original data contains over 700000 rows each representing one of seven types of transactions in AAVE protocol. Each row contains over 30 columns (some fields might not be used for some transaction types) that represent different aspects of transactions made by users.

In order to use this data for liquidation analysis (for good and deficient liquidations), we had to build a few new data frames out of the existing transactions data. You can read more about each of them in the appropriate sections.
...
```{r, echo = FALSE}
#data collection is the same as in all other notebooks
...
For deficient liquidation analysis, we used two different dataframes. Both dataframes were based on original transaction data. The first dataframe contains deficient liquidation transactions only. There are 229 rows in this data set and 33 columns (same as in raw transactions data). We mostly used columns associated with collateral and principal (liquidation collateral in USD, liquidation principal in USD, collateral and principal cryptocurrency names and types, etc.). Similarly, we constructed a separate dataframe used for deficient liquidators. This dataframe was based on the deficient liquidation dataframe and was used as a summary of their activity. The dataframe has 154 rows and 10 columns that contain information on total and average collateral and principal used by these users in deficient liquidations, total number of deficient transactions, etc. 

The data frames were analyzed from various points of view, different statistics were collected and reflected on the graphs and tables.
...
plot1 <- ggplot(dfst,aes(x = amountUSDPincipal, y = amountUSDCollateral,color = collateralType, shape = principalType)) + geom_point() + ggtitle("Liquidations with Collateral below Principal") + geom_abline() + xlab("Principal (USD)") + ylab("Collateral (USD)") + scale_y_continuous(labels = scales::comma) + scale_x_continuous(labels = scales::comma)
```

```{r def_1, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:def_1} Distribution of Deficient Liquidation Transactions"}
plot1

```

We can take a look into how deficient liquidations are distributed on Figure \ref{fig:def_1}. The diagonal line represents the identity line where collateral = principal, obviously, all of deficient liquidations are below this line. First of all, there are just a few liquidations with very high collateral and principal value. Most of the deficient liquidations are below $100000 in principal. Second, we see that there are no deficient liquidations, where both collateral and principal are stable coins. This is probably just because there are very little (stable,stable) liquidations in general. Lastly, we observe that some complicated distribution in terms of distance from the identity line. There are some deficient liquidations that are extremely close to the identity line, but there are also a lot of deficient liquidations that are quite distant from it.
...
plot <- ggplot(dfst,aes(y = total_collateral,x = total_principal,color = num_def)) + geom_point() + ggtitle("Deficient Liquidators") + geom_abline() + labs(title = "Deficient Liquidators", x = "Total Collateral(USD)", y = "Total Principal(USD)", color = "Total transactions") + scale_y_continuous(labels = scales::comma) + scale_x_continuous(labels = scales::comma)
...
defusers_liq <- defusers_liq %>% group_by(user_alias) %>% summarize(num = n(), num_def = sum(ifelse(deficiency,1,0))) %>% mutate(percent_def = num_def/num * 100)
```

``` {r table, echo=FALSE}
kable(head(defusers_liq,10), caption = "Deficient Liquidators Dataframe (Part)")
```
...
Similar to deficient liquidation analysis, we had to contruct new dataframe for good liquidators. The dataframe only contains information about good liquidators and was built directly from raw transaction data. The dataframe has 154 rows and 10 columns that contain information on total and average collateral and principal used by these users in good liquidations, profit they made, total number of liquidation transactions, etc. 

This data frame was analyzed from various points of view, different statistics were collected and reflected on the graphs and tables.
...
plot_liq <- dfl %>% ggplot(aes(x = total_collateral, y = num_transact, color = total_profit)) + geom_point() + scale_color_gradientn(colours = rainbow(5), labels = scales::comma) + labs(title = "Liquidation Strategy of Good Liquidators", x = "Total Collateral in Liquidations(USD)", y =  "Number of Liquidations", color = "Total Profit (USD)") + scale_x_continuous(labels = scales::comma)
```

```{r good_1, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:good_1} Liquidations History of Good Liquidators"}
plot_liq
...
```{r good_2, echo=FALSE, fig.width=7,fig.height=6,fig.cap="\\label{fig:good_2} Comparison of Different Grops of Liquidators"}
plot

```

Figure \ref{fig:good_1} and Figure \ref{fig:good_2} show the information on best liquidators(liquidators that made at least 100000 USD profit from liquidations). The plot compares number of liquidations and total collateral claimed in all of liquidations for best liquidators. Colors correspond to amount of profit (collateral - principal) gained in these transactions.
...
First, it looks like liquidations can be used as great a way to make a lot of profit. We observe not only a lot of immediately-profitable liquidations, but also some deficient liquidations, which may be used to make a lot of profit outside AAVE. 

Second, there are obviously different groups of liquidators and we can see that different groups have different patterns of behavior. We can study and analyze these patterns in more detail (some analysis is already present in this paper, but there is a lot more things to study). 

Lastly, it seems like being a profitable AAVE user means being a knowledgeable and careful user, which makes a lot of different transactions in AAVE. Most profitable liquidators are good AAVE users in general, so they not only make profit from liquidations, but also from other transactions. Experience and knowledge of the protocol is one of the keys of being a good user.
