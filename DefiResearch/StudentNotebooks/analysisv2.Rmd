...
wethLiquids <- liquidations %>%
  ...
  dplyr::select(user,date,reservePriceUSDCollateral,timestamp,collateralReserve)

wethBorrows <- borrows %>% 
  dplyr::filter(reserve=="WETH") %>%
  dplyr::select(onBehalfOf,date,reservePriceUSD,timestamp,borrowRateMode) %>%
...
head(km3,25)
km_fit_cr <- survfit(Surv(timeDiff,status) ~ collateralReserve, data=wethBorrowLiquids)
