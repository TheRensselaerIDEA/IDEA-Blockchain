---
title: "DAR Defi Team Assignment 5 (Fall 2021)"
...
df.liquidation <- df %>% 
  ...
  filter(amountUSDCollateral <= 10000000) %>%
  filter(1 <= amountUSDCollateral) %>%
  filter(amountUSDPincipal <= 10000000) %>%
  filter(1 <= amountUSDPincipal)
head(df.liquidation)
summary(df.liquidation$principalReserve)

summary(df.liquidation$collateralReserve)
head(df.borrow)
suser <- sample(df.liquidation$user,1)
suser
df.user <- df %>% filter(user == suser)
df.user <- df.user[order(df.user$date),]
df.user
df.liquidation["ratio"] = df.liquidation$amountUSDPincipal/df.liquidation$amountUSDCollateral
summary(df.liquidation$ratio)
df.strange <- df.liquidation %>% filter(ratio>=1)
df.regular <- df.liquidation %>% filter(0<ratio) %>%
  filter(ratio<1)
ggplot(df.regular, aes(ratio))+
  geom_density()
ggplot(df.strange, aes(ratio))+
  geom_density()
```
```{r}
df.wholel <- filter(df, user %in% df.liquidation$user)
summary(df.wholel)
df.wholel <- df.wholel %>% filter(!is.na(borrowRate)) %>% 
  filter(type == "borrow") %>% 
  filter(borrowRate <= 50) %>% 
  filter(amountUSD <= 10000000) %>%
  filter(1 <= amountUSD)

df.borrow.new2 <- df.wholel %>%
...
ggplot(df.borrow.new2,aes(borrowRatePercentage, color=borrowRateMode)) +
  ggtitle("Borrow rate for top coins")+ 
  geom_density()

ggplot(df.borrow.new2,aes(log10(amountUSD), color=borrowRateMode)) +
...
df.test <- df.borrow %>% dplyr::select(c('borrowRate','amountUSD', 'user'))
str(df.test)
df.test["class"] = TRUE
for (i in 1:nrow(df.borrow)){
  df.test[i,4] = as.integer(df.test[i,3] %in% df.liquidation$user)*2-1
}
str(df.test)
summary(df.test)
class <- as.factor(df.test$class)
str(class)
...
```{r}
df.test <- df.borrow %>% dplyr::select(c('borrowRate','amountUSD'))
n <- nrow(df.test)
ss <- ceiling(n*0.90)
...
train <- df.test %>% slice(train.perm) 
validation <- df.test %>% slice(-train.perm)
