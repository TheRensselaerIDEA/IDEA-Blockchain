...
The bulk of our research can be summed into transaction density plots of the various clusters (see appendix and referenced kwond2_assignment4.Rmd for more analysis). 

For this iteration's set of hyperparameters, we were able to characterize a group of highly active swaps, especially within May, June, July for cluster 1, a smaller group of borrows for cluster 2, and slightly risky/liquid users in cluster 4. In one instance, we had multiple high risk groups that were able to be characterized (view appendix).
...
pgg <- gg_color_hue(6)
cluster_names <- c('0' = "Cluster 1", '1' = "Cluster 2", '2' = "Cluster 3", '3' = "Cluster 4", '4' = "Cluster 5", '5' = "Cluster 6")
...
Here, we provide an alternative run of our process which shows a few high risk/liquid groups. 
```{r, out.width="5\\linewidth", include=TRUE, fig.align="center", fig.cap=c("Fig 4: Alternative Transaction Types Over Time by UMAP+HDBSCAN User Clusters"), echo=FALSE}
...
Generally, it seems to be that our new features fall short, without proper tuning of the number of UMAP dimensions and the min points in HDBSCAN. However, we are certainly able to cluster users with specific behaviors to a weekly transaction scale as shown above, especially if we observe Fig 4. The clustering was able to group two large liquidation groups of users, Cluster 5 and 6, and more stable borrows such as clusters 2 and 3 (though it looks like our clustering located time as a pattern, as cluster 3 transactions happen in late May).
...
It terms of interpreting the survival plots, it seems that users from cluster 3 generally repay faster/repay in general compared to the overall dataset. We discuss more about figures 5-8 below.
...
print(generateSurvival(borrows, repays, paste("Fig", i+4,": Cluster",i,": First Borrow to Repay Survival Analysis") ))
  }
...
print(generateSurvival(borrows, repays, paste("Fig", i+5, "Entire Dataset: First Borrow to Repay Survival Analysis") ))
