...
Below are the borrow rates and amounts for several different coins.
...
df.borrow.PAX <- df.borrow %>% filter(reserve == "PAX")
#summary(df.borrow.PAX)
ggplot(df.borrow.PAX,aes(borrowRate, color = "PAX")) +
  ggtitle("User Borrow History")+ 
  geom_density()

ggplot(df.borrow.PAX,aes(log10(amountUSD), color = "PAX")) +
  ggtitle("User Borrow History")+ 
  geom_density()
...
```{r}
ggplot(df.borrow,aes(borrowRate, color = reserve)) +
  ggtitle("User Borrow History")+ 
  geom_density()

ggplot(df.borrow,aes(log10(amountUSD), color = reserve)) +
  ggtitle("User Borrow History")+ 
  geom_density()

df.borrow.select <- df.borrow %>% 
  filter(reserve == "LINK" | reserve == "PAX" | reserve == "USDC" | reserve == "DAI" | reserve == "BUSD" | reserve == "AMPL")

ggplot(df.borrow.select,aes(borrowRate, color = reserve)) +
  ggtitle("User Borrow History")+ 
  geom_density()+
  facet_grid(rows = df.borrow.select$reserve)

ggplot(df.borrow.select,aes(log10(amountUSD), color = reserve)) +
  ggtitle("User Borrow History")+ 
  geom_density()+
  facet_grid(rows = df.borrow.select$reserve)

df.borrow.select <- df.borrow %>% 
  filter(reserve == "PAX" | reserve == "USDC" | reserve == "DAI" | reserve == "BUSD" | reserve == "AMPL")
