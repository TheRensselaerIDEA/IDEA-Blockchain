...
```{r}
x <- mean(all_redeems$value) / mean(all_redeems$count)

ggplot(all_redeems, aes(x = date))+ 
  geom_line(aes(y = value, colour = "Amount"))+
  geom_line(aes(y = count*x, colour = "# Transactions"))+
  scale_y_continuous(labels = comma, sec.axis = sec_axis(~./x, name = "# of Redeems"))+
  scale_colour_manual(values = c("blue", "red"))+
  labs(y = "Amount (USD)",x = "Month",colour = "Parameter")+
  scale_x_date(date_breaks = "2 months", date_labels = "%b-%y")+
  theme(legend.position = c(0.9, 0.9))+
  ggtitle("Redeems Summary")
```

## Build Utilization Charts
...
```{r}
head(df%>%
       filter(type == "liquidation" & principalReserve == "BUSD"))
```
