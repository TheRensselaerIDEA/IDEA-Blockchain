...
8. Confirm what you just did; make the following additional edits:
   * What is the location of the github: _https://github.rpi.edu/DataINCITE/IDEA-Blockchain/tree/dar-mink3_
   * What is your github ID: _mink3_
   * What is the name of your new branch: _dar-mink3_
   * What is the name of your new (copied) notebook: _mink3-assignment1-f21.Rmd_
...
```{r}
users <- plyr::count(as.factor(df$user))
top20transactionuser <- users[order(users$freq, decreasing = TRUE),]$x[1:20]

df.1users<-filter(df, user == top20transactionuser[1])
...
df.1users %>% 
  dplyr::count(ymd_new, type, name = "amount") %>% 
ggplot() + 
  geom_bar(aes(x=ymd_new,y= amount, fill = type),
           stat='identity') + ggtitle("Single User Transactions Weekly") + labs(y = "Number of transactions", x= "Time (Weeks)")+ 
  theme(
           axis.text.x=element_blank(),
           axis.ticks.x=element_blank())

df.1users %>% 
  dplyr::count(ymd_new, type, wt=amountUSD, name = "amountUSD") %>% 
ggplot() + 
  geom_bar(aes(x=ymd_new,y= amountUSD, fill = type),
           stat='identity') + ggtitle("Single User Transactions amountUSDWeekly") + labs(y = "AmountUSD of transactions", x= "Time (Weeks)")+ 
  theme(
           axis.text.x=element_blank(),
           axis.ticks.x=element_blank())
...
```{r}
users <- plyr::count(df, vars = "user", wt_var="amountUSD")
top20amountuser <- users[order(users$freq, decreasing = TRUE),]$user[1:20]

df.1users<-filter(df, user == top20amountuser[1])
...
ggplot() + 
  geom_bar(aes(x=ymd_new,y= amountUSD, fill = type),
           stat='identity') + ggtitle("Single User Transactions amountUSD Weekly") + labs(y = "AmountUSD of transactions", x= "Time (Weeks)")+ 
  theme(
           axis.text.x=element_blank(),
           axis.ticks.x=element_blank())
